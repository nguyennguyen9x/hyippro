    ...
    let SlotCayKheController = SlotCayKheController_1 = class SlotCayKheController extends cc.Component {
           ...
           case SlotCayKhe_Cmd_1.default.Code.FREE_GAME:
            {
              ...
              let freeNumber = this.resultFreeSpin.object.length;
      ...
      showPopupGuide() {
        ...
        if (!popup) {
          ...
          cc.log(SlotCayKheController_1.infoItems);
          cc.log(SlotCayKheController_1.infoAwards);
          cc.sys.localStorage.setItem("guide", JSON.stringify(guide));
        ...
        if (!popup.dataLine) {
          popup.dataLine = [];
          for (let index = 1; index <= 25; index++) popup.dataLine[index] = {
            index: index,
            cells: [ 0, 0, 0, 0, 0 ].map(cell => Math.floor(3 * Math.random()))
          };
          popup.initLine();
      ...
      hideSelectGame(evt, data) {
        cc.tween(this.scrSelectGame).call(() => {
          cc.log(this.freeSpinOption[data]);
    ...
    let PopupGuide = class PopupGuide extends Dialog_1.default {
      ...
      init(data = this.data, listSpriteFrame) {
        ...
        data.info.forEach(content => {
          ...
          let str = content.guide.sort((a, b) => b.award.duplicate - a.award.duplicate).map(a => a.award.duplicate > 0 ? "X" + a.award.duplicate + " " + a.text : a.text).join("\n");
