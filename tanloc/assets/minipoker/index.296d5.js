    ...
    let MiniPokerController = class MiniPokerController extends MiniGame_1.default {
      ...
      onSpinResult(data) {
        ...
        for (let i = 0; i < this.columns.childrenCount; i++) {
          ...
          roll.runAction(cc.sequence(cc.delayTime(.2 * i * timeScale), cc.moveTo(.2 * timeScale, cc.v2(roll.getPosition().x, step1Pos)).easing(cc.easeQuadraticActionOut()), cc.moveTo((this.spinDuration + this.addSpinDuration * i) * timeScale, cc.v2(roll.getPosition().x, step2Pos)).easing(cc.easeQuadraticActionInOut()), cc.moveTo(.2 * timeScale, cc.v2(roll.getPosition().x, step3Pos)).easing(cc.easeQuadraticActionIn()), cc.callFunc(() => {
            roll.setPosition(cc.v2(roll.getPosition().x, 0));
            i === this.columns.childrenCount - 1 && this.spined();
          })));
          roll.runAction(cc.sequence(cc.delayTime((.45 + .2 * i) * timeScale), cc.callFunc(() => {
