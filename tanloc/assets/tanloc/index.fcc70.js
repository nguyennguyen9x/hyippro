window.__require = function e(t, n, r) {
  ...
  "TanLoc.Controller": [ function(require, module, exports) {
    ...
    let TanLocController = TanLocController_1 = class TanLocController extends cc.Component {
      constructor() {
        ...
        this.listUserWin = [];
        this.guideContent = "\u0110\u1ec3 c\xf3 th\u1ec3 tham gia,\nB\u1ea1n c\u1ea7n s\u0103n ticket khi\nn\u1ea1p ti\u1ec1n \u0111\u1ea1t c\xe1c m\u1ed1c\n%cash_in\nt\u01b0\u01a1ng \u01b0\u1edbng v\u1edbi \u0111\u01b0\u1ee3c t\u1eb7ng\n%ticket ticket\nTICKET C\xc0NG NHI\u1ec0U,\nC\u01a0 H\u1ed8I C\xc0NG L\u1edaN";
      }
      static getInstance() {
      ...
      start() {
        ...
        this.btnBingo.node.active = false;
           ...
           case TanLoc_Cmd_1.default.Code.NEW_USER_WIN:
            {
              ...
              this.listUserWin.find(data => data.coin_win == res.coin_win && data.name == res.name) || this.listUserWin.push(res);
              1 == this.listUserWin.length && this.scheduleOnce(() => {
                this.playerWin();
              }, 1);
            }
            break;
           ...
           case TanLoc_Cmd_1.default.Code.CHANGE_STEP:
            {
              ...
              this.setCountDown(res.countDown);
              this.soundControler.playSoundCountdown(res.countDown);
              "WAIT" == this.stepGame ? this.scheduleOnce(() => {
                this.requestConfig();
                this.requestUserInfo();
              }, 1) : "REGISTER" == this.stepGame ? this.scheduleOnce(() => {
                this.dismissChat();
      ...
      playerWin() {
        for (let i = 0; i < Math.min(8, this.listUserWin.length); i++) {
          let lb = this.lbUserWin.parent.children.find(chil => !chil.active);
          if (!lb) continue;
          let dataWin = this.listUserWin.shift();
          lb.getComponent(cc.Label).string = "Ch\xfac m\u1eebng " + dataWin.name + " \u0111\xe3 tr\xfang " + Utils_1.default.formatNumber(dataWin.coin_win);
          cc.Tween.stopAllByTarget(lb);
          cc.tween(lb).set({
            x: cc.winSize.width / 2,
            active: true
          }).to(5, {
            x: -cc.winSize.width / 2 - lb.width
          }).set({
            active: false
          }).start();
        }
        this.listUserWin.length > 0 && this.scheduleOnce(() => {
          this.playerWin();
        }, 1);
      }
      resizePlayView() {
      ...
      requestConfig() {
        Http_1.default.get(Configs_1.default.App.API_TAN_LOC + "config", {}, (err, res) => {
          ...
          for (const key in res.configRechargeMoneyTicket.rangeValues) if (Object.prototype.hasOwnProperty.call(res.configRechargeMoneyTicket.rangeValues, key)) {
            ...
            lbCashIn += Utils_1.default.formatMoneyChip(key) + ", ";
            ticket += element + ", ";
          }
          this.cfgGuide.string = this.guideContent.replace("%cash_in", lbCashIn).replace("%ticket", ticket);
          (res.countDown > 0 || "WAIT" == this.stepGame) && this.scheduleOnce(() => {
            this.requestConfig();
          }, 1);
          if ("WAIT" == this.stepGame) {
            if (!this.dataInfo.rangeValues) {
              this.actionChat();
      ...
      initProcess() {
        let max = 0;
        for (const key in this.dataInfo.rangeValues) max = Math.max(parseInt(key), max);
        if (this.dataInfo.rangeValues && this.dataInfo.todayRecharge >= 0) {
          let max = 0;
          for (const key in this.dataInfo.rangeValues) max = Math.max(parseInt(key), max);
          this.processWanting.fillRange = this.dataInfo.todayRecharge / max;
        }
        let ind = 0;
        for (const key in this.dataInfo.rangeValues) if (Object.prototype.hasOwnProperty.call(this.dataInfo.rangeValues, key)) {
          ...
          if (parseInt(key) > 0 && element > 0) {
            ...
            let lbProcess = this.processWanting.node.children[ind];
            if (!lbProcess) {
              lbProcess = cc.instantiate(this.lbProcessWanting);
              this.processWanting.node.addChild(lbProcess);
              lbProcess.opacity = 0;
            }
            lbProcess.getComponent(cc.Label).string = element + "\n\n\n" + Utils_1.default.formatMoneyChip(key);
            lbProcess.active = true;
            lbProcess.x = parseInt(key) / max * 500 + 20;
            cc.tween(lbProcess).delay(.5).to(.5, {
      ...
      setCountDown(countDown) {
        ...
        !this.wantingNode.active && isWaitTime && this.scheduleOnce(this.initProcess, 1);
      ...
      actionChat() {
        ...
        cc.tween(this.panelChat.node).to(.5, {
          ...
          opacity: 255,
          height: this.gamePlay.height - 500
        }, {
          easing: "quartInOut"
        }).start();
      }
    ...
    __decorate([ property(cc.Label) ], TanLocController.prototype, "cfgGuide", void 0);
  ...
  "TanLoc.PopupHistory": [ function(require, module, exports) {
    ...
    let TanLocPopupHistory = class TanLocPopupHistory extends TanLoc_Dialog_1.default {
      constructor() {
        ...
        this.lbPage = null;
        this.pageViewHistory = null;
        this.scrollInfoSpin = null;
        this.itemInfoSpin = null;
        this.ticketInfoSpin = null;
        this.itemResult = null;
        this.pageSize = 10;
        this.currentPage = 0;
      }
      ...
      loadInfoSpin(history) {
        let date = history["date"][0] + "-" + (history["date"][1] < 10 ? "0" : "") + history["date"][1] + "-" + (history["date"][2] < 10 ? "0" : "") + history["date"][2];
        this.scrollInfoSpin.content.removeAllChildren(true);
        Http_1.default.get(Configs_1.default.App.API_TAN_LOC + "history_bingo_round", {
          session: history["session"]
        }, (err, res) => {
          cc.log(res);
          this.pageViewHistory.scrollToPage(1, .5);
          this.setTicket(this.ticketInfoSpin, history);
          res.forEach(round => {
            let item = cc.instantiate(this.itemInfoSpin);
            this.scrollInfoSpin.content.addChild(item);
            item.getChildByName("time").getComponent(cc.Label).string = (round["startSpinTime"][3] < 10 ? "0" : "") + round["startSpinTime"][3] + ":" + (round["startSpinTime"][4] < 10 ? "0" : "") + round["startSpinTime"][4] + ":" + (round["startSpinTime"][5] < 10 ? "0" : "") + round["startSpinTime"][5] + " " + (round["startSpinTime"][2] < 10 ? "0" : "") + round["startSpinTime"][2] + "/" + (round["startSpinTime"][1] < 10 ? "0" : "") + round["startSpinTime"][1] + "/" + (round["startSpinTime"][0] < 10 ? "0" : "") + round["startSpinTime"][0] + "\nRound: " + round["roundId"];
            item.getChildByName("lb_result").getComponent(cc.Label).string = this.checkWin(history["matrix"].split(","), round.numbers.split(",")) ? "Win" : "Lose";
            item.getChildByName("lb_result").getComponent(cc.Label).getComponent(cc.LabelOutline).color = history["reward"] ? cc.Color.BLACK.fromHEX("#FFA300") : cc.Color.BLACK.fromHEX("#FF01F0");
            item.active = true;
            this.ticketInfoSpin.getComponentInChildren(cc.ScrollView).content.removeAllChildren(true);
            item.off(cc.Node.EventType.TOUCH_START);
            item.on(cc.Node.EventType.TOUCH_START, () => {
              this.ticketInfoSpin.getComponentInChildren(cc.ScrollView).content.removeAllChildren(true);
              round.numbers.split(",").map(a => parseInt(a)).forEach(num => {
                let newResult = cc.instantiate(this.itemResult);
                newResult.active = true;
                newResult.getChildByName("number-ball").getComponent(cc.Label).string = (num <= 16 ? "B" : num <= 32 ? "I" : num <= 48 ? "N" : num <= 64 ? "G" : "O") + (num < 10 ? "\n0" + num : "\n" + num);
                this.ticketInfoSpin.getComponentInChildren(cc.ScrollView).content.addChild(newResult);
              });
              this.ticketInfoSpin.getChildByName("ticket").children.forEach(chil => {
                let spr_win = chil.getComponentInChildren(cc.Sprite);
                spr_win && (spr_win.node.active = false);
              });
              history["matrix"].split(",").forEach((num, ind) => {
                let spr_win = this.ticketInfoSpin.getChildByName("ticket").getChildByName("" + ind).getComponentInChildren(cc.Sprite);
                spr_win && -1 != round.numbers.split(",").indexOf(num) && (spr_win.node.active = true);
              });
              this.ticketInfoSpin.getComponentInChildren(cc.ScrollView).scrollToTop(.2);
            });
          });
        });
      }
      checkWin(ticket, result) {
        let listItemWin = [ 12 ];
        ticket.forEach((num, idx) => {
          -1 !== result.indexOf(num) && listItemWin.push(idx);
        });
        let listTicketCheck = [];
        for (let i = 0; i < 5; i++) {
          listTicketCheck = [];
          for (let j = 0; j < 5; j++) -1 != listItemWin.indexOf(5 * i + j) && listTicketCheck.push(5 * i + j);
          if (5 == listTicketCheck.length) return true;
          listTicketCheck = [];
          for (let j = 0; j < 5; j++) -1 != listItemWin.indexOf(i + 5 * j) && listTicketCheck.push(i + 5 * j);
          if (5 == listTicketCheck.length) return true;
        }
      ...
      infoSpinToTop() {
        this.scrollInfoSpin.scrollToTop(.5, false);
      }
      loadData(page) {
        this.scrollHistory.content.destroyAllChildren();
        Http_1.default.get(Configs_1.default.App.API_TAN_LOC + "historytickets", {
          ...
          this.lbPage.string = this.currentPage + 1 + "";
          if (0 === historys.length) return;
          this.scrollHistory.content.children.forEach(chil => {
            chil.off(cc.Node.EventType.TOUCH_START);
          });
          for (let i = 0; i < historys.length; i++) {
            const history = historys[i];
            if (!history["matrix"]) continue;
            let historyItem = cc.instantiate(this.itemHistory);
            this.scrollHistory.content.addChild(historyItem);
            this.setTicket(historyItem, history);
            historyItem.off(cc.Node.EventType.TOUCH_START);
            historyItem.on(cc.Node.EventType.TOUCH_START, () => {
              this.loadInfoSpin(history);
            });
          }
        });
      }
      setTicket(historyItem, history) {
        ...
        history["matrix"].split(",").forEach((num, ind) => {
          ...
          let spr_win = historyItem.getChildByName("ticket").getChildByName("" + ind).getComponentInChildren(cc.Sprite);
          spr_win && (spr_win.node.active = false);
        });
      ...
      back() {
        this.scrollInfoSpin.content.removeAllChildren(true);
        this.scrollInfoSpin.content.removeAllChildren(true);
        this.ticketInfoSpin.active = false;
        this.pageViewHistory.scrollToPage(0, .5);
      }
      actionPrePage() {
    ...
    __decorate([ property(cc.PageView) ], TanLocPopupHistory.prototype, "pageViewHistory", void 0);
    __decorate([ property(cc.ScrollView) ], TanLocPopupHistory.prototype, "scrollInfoSpin", void 0);
    __decorate([ property(cc.Node) ], TanLocPopupHistory.prototype, "itemInfoSpin", void 0);
    __decorate([ property(cc.Node) ], TanLocPopupHistory.prototype, "ticketInfoSpin", void 0);
    __decorate([ property(cc.Node) ], TanLocPopupHistory.prototype, "itemResult", void 0);
  ...
  "TanLoc.SoundController": [ function(require, module, exports) {
    ...
    let SoundControler = SoundControler_1 = class SoundControler extends cc.Component {
      ...
      playSound(sound = this.click, speed = 1, repeat = 1) {
